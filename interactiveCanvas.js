class InteractiveCanvas {
  ...
  setupContainer() {
    ...
    this.container.style.cursor = "crosshair"; // Changed from 'grab' since we don't support mouse drag
  ...
  setupEventListeners() {
    // Mouse events for right-click panning
    ...
    // Mouse wheel for zoom (requires Cmd key)
  ...
  handleMouseDown(e) {
    if (e.button !== 2) return; // Only handle right mouse button for panning
  ...
  // Wheel event for zoom (requires Cmd/Meta key)
  handleWheel(e) {
    if (!this.options.enableZoom) return;

    // Only zoom with Cmd/Meta key pressed
    ...
    // Moderate zoom speed
    const zoomFactor = delta > 0 ? 0.97 : 1.03; // Zoom out 10% or zoom in 10% per gesture
  ...
  handleTouchStart(e) {
    if (e.touches.length === 2) {
      // Two finger touch - prepare for pan or zoom (depending on cmd key)
      e.preventDefault();
      this.isDragging = true;
      ...
      this.lastPointerPos = this.lastPinchCenter;
    }
    // Ignore single touch - let it pass through to drawing
  }
  ...
  handleTouchMove(e) {
    if (e.touches.length === 2 && this.isDragging) {
      ...
      // Check if Cmd/Meta key is pressed for zoom mode
      if (e.metaKey || e.ctrlKey) {
        // Zoom mode with Cmd+two fingers - make it faster
        if (this.lastPinchDistance > 0 && this.options.enableZoom) {
          const rawZoomFactor = distance / this.lastPinchDistance;
          // Amplify the zoom change for extremely fast zooming
          const zoomFactor =
            rawZoomFactor > 1
              ? 1 + (rawZoomFactor - 1) * 50.0
              : 1 - (1 - rawZoomFactor) * 50.0;
        ...
        // Pan mode with two fingers (no modifier key)
        if (this.options.enablePan) {
          const deltaX = center.x - this.lastPointerPos.x;
          const deltaY = center.y - this.lastPointerPos.y;
  ...
  handleTouchEnd(e) {
    if (e.touches.length < 2) {
      // Reset when going below two fingers
      this.isDragging = false;
      this.lastPinchDistance = 0;
    }
    // Let single touches pass through to drawing
  }

  // Modern Pointer Events (handles mouse, touch, and pen uniformly)
  handlePointerDown(e) {
    // Only handle touch/pen pointers, ignore mouse
    if (e.pointerType === "mouse") return;

    this.pointers.set(e.pointerId, { x: e.clientX, y: e.clientY });

    if (this.pointers.size === 2) {
      // Two fingers down - prepare for pan or zoom
      e.preventDefault();
      this.isDragging = true;
    ...
    // Single pointer - let it pass through to drawing
  }
  ...
  handlePointerMove(e) {
    ...
    if (this.pointers.size === 2 && this.isDragging) {
      e.preventDefault();
  ...
  handlePointerUp(e) {
    ...
    // Let single pointers pass through to drawing
  }
  ...
  updateTransform() {
    if (this.content) {
      this.content.style.transform = `translate(${this.transform.x}px, ${this.transform.y}px) scale(${this.transform.scale})`;
    }
  ...
  destroy() {
    ...
    this.container.removeEventListener("contextmenu", (e) =>
      e.preventDefault()
    );
