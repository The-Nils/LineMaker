class Kaleidoscope {
    ...
    extractPathsFromSVG(element) {
        ...
        pathElements.forEach(pathEl => {
            ...
            if (pathData) {
                paths.push(this.parseSVGPath(pathData));
            }
        });
        ...
        commands?.forEach(command => {
            ...
            for (let i = 0; i < coords.length; i += 2) {
                if (!isNaN(coords[i]) && !isNaN(coords[i + 1])) {
                    points.push({
                        x: coords[i] / this.pixelsPerMm, // Convert back to mm
                        y: coords[i + 1] / this.pixelsPerMm,
                        move: type === 'M' || (type === 'L' && i === 0)
                    });
                }
            }
        });
    ...
    generateGcode(paths) {
        ...
        paths.forEach((path, pathIndex) => {
            ...
            path.forEach((point, pointIndex) => {
                if (point.move || pointIndex === 0) {
                    // Move to start point with pen up
                    gcode += `G0 Z${this.settings.penUpZ} ; Pen up\n`;
                    gcode += `G0 X${point.x.toFixed(3)} Y${point.y.toFixed(3)} F${this.settings.feedRate} ; Move to start\n`;
            ...
            gcode += `G0 Z${this.settings.penUpZ} ; Pen up at end of path\n\n`;
        });
